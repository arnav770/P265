...
# Code for Project 265 Grayscale start here
	...
	result = cv2.VideoWriter('static/'+'blackandwhie.mp4',cv2.VideoWriter_fourcc(*'mp4v'),
		30, size, 0)
	...
	try:
		...
		while True:
			...
			gray = cv2.cvtColor(frame_image, cv2.COLORBGR2GRAY)
			result.write(gray)
			video_file = 'blackandwhite.mp4'

	except:
		print("Complete reading all the frames from the video")
